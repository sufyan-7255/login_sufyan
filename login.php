...
<div class="login">
  ...
  <form class="login-container" method="POST">
    <p><input type="email" name="emailinput" placeholder="Email"></p>
    <p><input type="password" name="passinput" placeholder="Password"></p>
    <a><input type="submit" name="loginbtn" value="Log in"></a>
  </form>
</div>
</body>
</html>
...
if(isset($_POST["loginbtn"]))
{
  ...
  $loginquery =  "select * from login_tbl where email = '".$email."' and pass = '".$pass."'";
  $loginqueryconnect = mysqli_query($con, $loginquery);
  $count = mysqli_num_rows($loginqueryconnect);
  if($count>0)
  {

    $r = mysqli_fetch_array($loginqueryconnect);
    $_SESSION["namevar"] = $r["name"];
